...
<%@page import="com.dotcms.repackage.bsh.Interpreter"%>
<%@page import="com.dotmarketing.util.UtilMethods"%>
			...
			<tr>	
				...
				<% 
					if(ReportParamterBR.isNumberParameter(par.getClassType()) || ReportParamterBR.isStringParameter(par.getClassType())){
						...
						defaultValue = (String) new com.dotcms.repackage.bsh.Interpreter().eval(defaultValue);
						defaultValue = (UtilMethods.isSet(defaultValue) ? defaultValue : "");
				%>						
				...
				<%	}else if(ReportParamterBR.isDateParameter(par.getClassType())){
						...
						if(par.getDefaultValue() != null && !par.getDefaultValue().equals("")){
							 c.setTime((Date)new com.dotcms.repackage.bsh.Interpreter().eval(par.getDefaultValue()));
						}
				...
				%>
					...
					%>
						<td>
							<%= LanguageUtil.get(pageContext, "True") %> <input <% if(((Boolean)new com.dotcms.repackage.bsh.Interpreter().eval(par.getDefaultValue())).booleanValue()){ %>checked="checked"<%} %> type="radio" id="<%= par.getName() %>" name="<%= par.getName() %>" value="true" >
							<%= LanguageUtil.get(pageContext, "False") %> <input <% if(!((Boolean)new com.dotcms.repackage.bsh.Interpreter().eval(par.getDefaultValue())).booleanValue()){ %>checked="checked"<%} %> type="radio" id="<%= par.getName() %>" name="<%= par.getName() %>" value="false" >
						</td>
					<%	
					...
					%>
						<td>
							<%
								java.lang.Object obj = new com.dotcms.repackage.bsh.Interpreter().eval(par.getDefaultValue());
